...
export default (viewer: Viewer, markerList: marker[]) => {
  ...
  const prepareToExportVideo = () => {
    ...
    // function recordCanvas(canvas: any) {
    ...
    //   // }, 3000);
    // }
      const _display = videoElement.style.display;
      let ratio = 1;
      ...
      videoElement.addEventListener('play', () => {
        ...
        // get video ratio
        ratio = videoElement.videoHeight / videoElement.videoWidth;
        // recover videoEl display
        videoElement.style.display = _display;
        // create fabric image
        drawVideo();
        // start
        videoElement.play();
  ...
  markerList.forEach((item, index) => {
    ...
    let aaaaaaCanvas = prepareToExportVideo()
    console.log('aaaaaaCanvas', aaaaaaCanvas)
    let elelele = document.querySelector('.left')
    // console.log('aaaaaaCanvas', aaaaaaCanvas)
    elelele?.appendChild(aaaaaaCanvas)
    const maker = viewer.entities.add({
      position: position,
      billboard: {
        // image: canvasPanel,
        // image: aaaaaaCanvas,
        image: document.getElementById('canvasEle'),
        // image: videoElement,
